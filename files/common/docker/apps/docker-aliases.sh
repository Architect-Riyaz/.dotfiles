alias dcb="docker compose up -d --build --force-recreate"
alias dclean='docker stop $(docker ps -q) 2>/dev/null; docker rm $(docker ps -aq) 2>/dev/null'

if [ -z "${DOCKER_APPS_DATA_PATH+x}" ]; then
  export DOCKER_APPS_DATA_PATH="$HOME/apps/docker-apps"
fi

# Ensure base path exists
mkdir -p "$DOCKER_APPS_DATA_PATH"

# ==============================
# Helper: Wait for TCP port
# ==============================
wait_for_port() {
  local HOST=$1
  local PORT=$2
  local SERVICE_NAME=$3
  local TIMEOUT=${4:-30}

  echo "Waiting for $SERVICE_NAME to be ready on $HOST:$PORT ..."

  for ((i=1; i<=TIMEOUT; i++)); do
    if nc -z "$HOST" "$PORT" >/dev/null 2>&1; then
      echo "$SERVICE_NAME is ready."
      return 0
    fi
    sleep 1
  done

  echo "ERROR: $SERVICE_NAME did not become ready within ${TIMEOUT}s."
  return 1
}
# ==============================
# PostgreSQL
# ==============================
docker-pg() {
  local PORT=${1:-5432}
  local APP_PATH="$DOCKER_APPS_DATA_PATH/postgres"

  mkdir -p "$APP_PATH"
  docker rm -f postgres >/dev/null 2>&1

  docker run -d \
    --name pg \
    --restart unless-stopped \
    -p ${PORT}:5432 \
    -e POSTGRES_USER=admin \
    -e POSTGRES_PASSWORD=strongpassword \
    -e POSTGRES_DB=appdb \
    -v "$APP_PATH":/var/lib/postgresql/data \
    postgres:latest

  # Check container running
  if ! docker ps --filter "name=^postgres$" --filter "status=running" | grep -q postgres; then
    echo "ERROR: PostgreSQL container is not running."
    return 1
  fi

  # Wait for service readiness
  wait_for_port "localhost" "$PORT" "PostgreSQL"
}

# ==============================
# MongoDB
# ==============================
docker-mongo() {
  local PORT=${1:-27017}
  local APP_PATH="$DOCKER_APPS_DATA_PATH/mongodb"

  mkdir -p "$APP_PATH"
  docker rm -f mongodb >/dev/null 2>&1

  docker run -d \
    --name mongo \
    --restart unless-stopped \
    -p ${PORT}:27017 \
    -e MONGO_INITDB_ROOT_USERNAME=admin \
    -e MONGO_INITDB_ROOT_PASSWORD=strongpassword \
    -v "$APP_PATH":/data/db \
    mongo:latest

  # Check container running
  if ! docker ps --filter "name=^mongodb$" --filter "status=running" | grep -q mongodb; then
    echo "ERROR: MongoDB container is not running."
    return 1
  fi

  # Wait for service readiness
  wait_for_port "localhost" "$PORT" "MongoDB"
}
# ==============================
# pgAdmin
# ==============================
docker-pgadmin() {
  local PORT=${1:-5050}
  local APP_PATH="$DOCKER_APPS_DATA_PATH/pgadmin"
  echo $APP_PATH;
  mkdir -p "$APP_PATH"
  docker rm -f pgadmin >/dev/null 2>&1

  docker run -d \
    --name pgadmin \
    --restart unless-stopped \
    -p ${PORT}:80 \
    -e PGADMIN_DEFAULT_EMAIL=admin@example.com \
    -e PGADMIN_DEFAULT_PASSWORD=strongpassword \
    -v "$APP_PATH":/var/lib/pgadmin \
    dpage/pgadmin4

  # Check container running
  if ! docker ps --filter "name=^pgadmin$" --filter "status=running" | grep -q pgadmin; then
    echo "ERROR: pgAdmin container is not running."
    return 1
  fi

  # Wait for service readiness
  wait_for_port "localhost" "$PORT" "pgAdmin"
}

# ==============================
# Redis
# ==============================
docker-redis() {
  local PORT=${1:-6379}
  local APP_PATH="$DOCKER_APPS_DATA_PATH/redis"

  mkdir -p "$APP_PATH"
  docker rm -f redis >/dev/null 2>&1

  docker run -d \
    --name redis \
    --restart unless-stopped \
    -p ${PORT}:6379 \
    -v "$APP_PATH":/data \
    redis:alpine \
    redis-server --appendonly yes

  # Check container running
  if ! docker ps --filter "name=^redis$" --filter "status=running" | grep -q redis; then
    echo "ERROR: Redis container is not running."
    return 1
  fi

  # Wait for service readiness
  wait_for_port "localhost" "$PORT" "Redis"
}

# ==============================
# DBeaver (Web)
# ==============================
docker-dbeaver() {
  local PORT=${1:-8978}
  local APP_PATH="$DOCKER_APPS_DATA_PATH/dbeaver"

  mkdir -p "$APP_PATH"
  docker rm -f dbeaver >/dev/null 2>&1

  docker run -d \
    --name dbeaver \
    --restart unless-stopped \
    -p ${PORT}:8978 \
    -v "$APP_PATH":/opt/dbeaver/workspace \
    dbeaver/cloudbeaver:latest

  if ! docker ps --filter "name=^dbeaver$" --filter "status=running" | grep -q dbeaver; then
    echo "ERROR: DBeaver container is not running."
    return 1
  fi

  wait_for_port "localhost" "$PORT" "DBeaver"
}

# ==============================
# Kali Linux
# ==============================
docker-kali() {
  local APP_PATH="$DOCKER_APPS_DATA_PATH/kali"
  local IMAGE_NAME="kali-with-tools"

  mkdir -p "$APP_PATH"

  echo "Building Kali image with tools (only needed once)..."
  docker build -t "$IMAGE_NAME" -f "$HOME/.docker/apps/kali.Dockerfile" $HOME/.docker/apps

  docker rm -f kali >/dev/null 2>&1

  docker run -dit \
    --name kali \
    --hostname kali \
    --restart unless-stopped \
    -v "$APP_PATH":/root/workspace \
    --cap-add=NET_ADMIN \
    --cap-add=SYS_PTRACE \
    "$IMAGE_NAME" \
    bash

  # Check container running
  if ! docker ps --filter "name=^kali$" --filter "status=running" | grep -q kali; then
    echo "ERROR: Kali container is not running."
    return 1
  fi

  echo "Kali Linux container started."
  echo "Attach using: docker exec -it kali bash"
}
